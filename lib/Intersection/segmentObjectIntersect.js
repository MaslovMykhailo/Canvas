...
const segmentCircleIntersect = (segment, circle) => {
  ...
  let directx = segment.y1 - segment.y0;
  let directy = segment.x0 - segment.x1;
  ...
  if(directx !== 0) {
      normalx = 100;
      normaly = (directy/directx) * (normalx - circle.x) + circle.y;
  } else {
      normaly = 100;
      normalx = (directx/directy) * (normaly - circle.y) + circle.x;
  }

  let normal = new Segment(normalx, normaly, circle.x, circle.y);
